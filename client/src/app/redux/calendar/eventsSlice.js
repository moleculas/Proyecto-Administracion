...
export const getEvents = createAsyncThunk(
  ...
  async (_, { getState, dispatch }) => {
    ...
    try {
      const response = await axios.get('/calendar/events/' + user.data.id);
...
export const addEvent = createAsyncThunk(
  ...
  async (newEvent, { getState, dispatch }) => {
    ...
    try {
      const response = await axios.post('/calendar/events', formData);
      const data = await response.data;
      return data;
    } catch (err) {
      dispatch(showMessage({ message: err.response.data.message, variant: "error" }));
...
export const updateEvent = createAsyncThunk(
  ...
  async (event, { dispatch }) => {
    ...
    formData.append("datos", JSON.stringify(losDatos));
    try {
      const response = await axios.put(`/calendar/events/${event.id}`, formData);
      const data = await response.data;
      return data;
    } catch (err) {
      dispatch(showMessage({ message: err.response.data.message, variant: "error" }));
