...
export const getLabels = createAsyncThunk(
  ...
  async (_, { getState, dispatch }) => {
    ...
    try {
      const response = await axios.get('/notes/labels/' + user.data.id);
...
export const createLabel = createAsyncThunk(
  ...
  async (newLabel, { getState, dispatch }) => {
    ...
    try {
      const response = await axios.post(`/notes/labels`, formData);
      const data = await response.data;
      return data;
    } catch (err) {
      dispatch(showMessage({ message: err.response.data.message, variant: "error" }));
...
export const updateLabel = createAsyncThunk(
  ...
  async (label, { getState, dispatch }) => {
    ...
    formData.append("datos", JSON.stringify(losDatos));
    try {
      const response = await axios.put(`/notes/labels/${label.id}`, formData);
      const data = await response.data;
      return data;
    } catch (err) {
      dispatch(showMessage({ message: err.response.data.message, variant: "error" }));
...
export const removeLabel = createAsyncThunk(
  ...
  async (id, { dispatch }) => {
    try {
      const response = await axios.delete(`/notes/labels/${id}`);
      const data = await response.data;
